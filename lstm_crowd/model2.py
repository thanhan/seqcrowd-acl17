...
class Model(object):
    ...
    def build(self,
        ...
        if crowd_dim:
            crowd_layer = EmbeddingLayer(n_crowds, crowd_dim, name='crowd_layer')
            crowd_scores = T.switch(T.neq(is_train, 0), crowd_layer.link(crowd_ids), 0*crowd_layer.link(crowd_ids))
        ...
        final_layer = HiddenLayer(word_lstm_dim + crowd_dim, n_tags, name='final_layer',
                                      activation=(None if crf else 'softmax'))
        ...
        if crowd_dim:
            ...
            cost = cost + (crowd_reg * crowd_layer.params[0] * crowd_layer.params[0]).mean()
