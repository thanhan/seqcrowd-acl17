...
def extract_spans(pop_dir, d, fn):
    ...
    for line in f:
        ...
        if temp[0].strip() != 'Participants': continue
        #print temp
        try:
            s = int(temp[1])
            e = int(temp[2])
...
def worker_quality(cd, gold, n_workers = 319):
    res = []
    for i in range(n_workers): res.append({})
    
    import hmm
    for g in gold:
        i = g[0]
        for cl in cd.crowdlabs[i]:
            wid = cl.wid
            sen = cl.sen
            res[wid][i] = sen
    ...
    for wid in range(n_workers):
        if len(res[wid]) > 0:
            r.append( (wid, len(res[wid]), hmm.eval_pico_new(gold, res[wid], l = 0, r = 1919, dic_res = True)) )
    return r
    return res
